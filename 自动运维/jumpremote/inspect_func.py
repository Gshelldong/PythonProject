...
def get_Host_Name(des_remote,cmd):  #得到主机名，并写入文件
    back_info = des_remote.get_cmd_output(cmd)
    with open("inspect_log.log", "a") as f:
        f.write("主机名:" + back_info + "| ")

def get_Remote_MenInfo(des_remote,cmd): # 得到内存信息，并写入文件
    back_info = des_remote.get_cmd_output(cmd)
    meminfo = float(back_info)
    if meminfo < kwords.MaxValue:
        meminfo = "%.0f%%" % (meminfo * 100)
        with open("inspect_log.log", "a") as f:
            f.write("内存:" + str(meminfo) + "| ")
    else:
        with open("inspect_log.log", "a") as f:
            f.write("内存异常请检查| ")


def get_Remote_CpuInfo(des_remote,cmd): # 得到CPU信息，并写入文件
    back_info = des_remote.get_cmd_output(cmd)
    back_info=float(back_info)
    if back_info < kwords.MaxValue * 100:
        with open("inspect_log.log", "a") as f:
            f.write("CPU使用率:" + str(back_info) + "%| ")
    else:
        with open("inspect_log.log", "a") as f:
            f.write("Cpu异常请检查| ")

def get_Remote_DiskInfo(des_remote,cmd):    #得到硬盘信息并写入文件
    back_info = des_remote.get_cmd_output(cmd)
...
def get_Sec(des_remote,cmd):    #得到今天的信安日志快照
    back_info = des_remote.get_cmd_output(cmd)
    back_info = int(back_info)
    if back_info > kwords.NormalValue:
        with open("inspect_log.log", "a") as f:
            f.write("信安日志快照:正常(参考值:" + str(back_info) + ")|\n")
    else:
        with open("inspect_log.log", "a") as f:
            f.write("信安日志快照异常\n")
...
def check_hadoop_proc(des_remote, cmd):   # 巡检进程 匹配关键字
    back_info = des_remote.get_cmd_output(cmd)
    fond_kwords_hadoop = re.compile(r"hadoop")
    fond_kwords = fond_kwords_hadoop.search(back_info)
    try:
        with open("inspect_log.log", "a") as f:
            f.write("hadoop进程正常(巡检关键字:"+fond_kwords.group()+")|")
    except AttributeError:
        print("目标主机程序运行异常请检查")

def get_hadoop_use(des_remote, cmd): # 校验master 进程master11-13
    back_info = des_remote.get_cmd_output(cmd)
    with open("inspect_log.log", "a") as f:
        f.write(back_info+" |")

def check_redis(des_remote, cmd):   #master 14-18 进程
    back_info = des_remote.get_cmd_output(cmd)
...
def check_60001(des_remote ,cmd):   # 检查60001端口
    back_info = des_remote.get_cmd_output(cmd)
    if int(back_info) > kwords.NormalValue:
        ...
        with open("inspect_log.log", "a") as f:
            f.write("60001端口异常请查看| ")

def sec_log(des_remote ,cmd):              #目前的信安日志
    back_info = des_remote.get_cmd_output(cmd)
    with open("inspect_log.log", "a") as f:
        f.write("信安日志:" + back_info + "| ")

def sec_log1(des_remote ,cmd):             #昨天的信安日志总量
    back_info = des_remote.get_cmd_output(cmd)
    with open("inspect_log.log", "a") as f:
        f.write("信安日志:" + back_info + "|\n")
...
def check_datanode_proc(des_remote, cmd):    # 校验存储服务器进程
    back_info = des_remote.get_cmd_output(cmd)
    ...
    try:
        ...
        with open("inspect_log.log", "a") as f:
            f.write("目标主机程序运行异常请检查|\n")
        print("目标主机程序运行异常请检查")

def inspect_ud40(cmd_list,inspect_fun,des_remote):  # ud40
    for cmd in cmd_list:  # 需要用的命令
        if cmd == cmd_list[0]:  # 主机名
            inspect_func.get_Host_Name(des_remote, cmd)
        elif cmd == cmd_list[1]:  # 内存
            inspect_func.get_Remote_MenInfo(des_remote, cmd)
        elif cmd == cmd_list[2]:  # cpu
            inspect_func.get_Remote_CpuInfo(des_remote, cmd)
        elif cmd == cmd_list[3]:  # 磁盘
            inspect_func.get_Remote_DiskInfo(des_remote, cmd)
        elif cmd == cmd_list[4]:  # 信安日志快照
            inspect_func.get_Sec(des_remote, cmd)
            
def inspect_ud45_ud46(cmd_list,des_remote):
    for cmd in cmd_list:  # 需要用的命令
        if cmd == cmd_list[0]:  # 主机名
            inspect_func.get_Host_Name(des_remote, cmd)
        elif cmd == cmd_list[1]:  # 内存
            ...
            inspect_func.check_hadoop_proc(des_remote, cmd)
        elif cmd == cmd_list[5]:  # 60001 端口
            inspect_func.check_60001(des_remote, cmd)
        elif cmd == cmd_list[6]:  # 今天信安日志
            inspect_func.sec_log(des_remote, cmd)
        elif cmd == cmd_list[7]:  # 昨天信安日志快照
            inspect_func.sec_log1(des_remote, cmd)

def inspect_ud50_ud52(cmd_list,des_remote):
    for cmd in cmd_list:  # 需要用的命令
        if cmd == cmd_list[0]:  # 主机名
            inspect_func.get_Host_Name(des_remote, cmd)
        elif cmd == cmd_list[1]:  # 内存
...
def inspect_namenode(cmd_list, des_remote):
    for cmd in cmd_list:
        if cmd == cmd_list[0]:
            inspect_func.get_Host_Name(des_remote, cmd)
        elif cmd == cmd_list[1]:  # 内存
...
def inspect_redis(cmd_list, des_remote):
    for cmd in cmd_list:
        if cmd == cmd_list[0]:  # 主机名
            inspect_func.get_Host_Name(des_remote, cmd)
        elif cmd == cmd_list[1]:  # 内存
            ...
            inspect_func.check_redis(des_remote, cmd)
            
def inspect_datanode(cmd_list, des_remote):
    for cmd in cmd_list:
        if cmd == cmd_list[0]:  # 主机名
            inspect_func.get_Host_Name(des_remote, cmd)
        if cmd == cmd_list[1]:  # 内存
            ...
            inspect_func.check_datanode_proc(des_remote, cmd)
