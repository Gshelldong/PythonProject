...
# 域名的zone_id
zone_ids = {'test1.com': '华为云域名zone_id',
           'test2': '华为云域名zone_id'}
...
def export_dns_record(zone_id):
    ...
    try:
        ...
        names_records = str((response.recordsets))
...
def get_cert_info(host_record):
    """
    这个函数憨域名证书的subject和commonname证书得到并返回
    :return:
    """
    # 设置socket的超时时间为5秒
...
if __name__ == '__main__':
    ...
    if choice in domain_id:
        choice_domain = domain_id.get(choice)
    ...
    name_records_len = len(name_records)
    for name_record in name_records:
        inspect_precent = (name_records.index(name_record) + 1 ) / name_records_len
        progress(inspect_precent)
        certs_inspect = []
        host_record = name_record['name'].rsplit('.',1)[0]
        certs_inspect.append(host_record)
        get_cert_info(host_record)
        get_cert_info_res = get_cert_info(host_record)

        if type(get_cert_info_res) is dict:
            # 表格的占位
        ...
        certs_inspect_res.append(certs_inspect)
    write_xlsx(certs_inspect_res)
    input('\n巡检完成,按任意键退出!')
